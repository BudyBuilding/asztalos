  ...
  try {
    ...
    const formatDate = (dateString) => {
      ...
      function getValues(items, key) {
        ...
        const values = [];
        arr.forEach(item => {
          if (item != null && item.hasOwnProperty(key) && !values.includes(item[key])) {
            ...
            {(
              workList.map((work) => (
                <tr key={work.workId} 
                    ...
                    style={{ cursor: "pointer", borderBottom: "thin solid #E9E7F1", height: "50" }}
                >
                  ...
                  <td style={cellStyle}>{formatDate(work.measureDate) || "--"}</td>
                  <td style={cellStyle}>{formatDate(work.ordereDate) || "--"}</td>
                  <td style={cellStyle}>{formatDate(work.finishDate) || "--"}</td>
                </tr>
              ))
            )}
          </tbody>
