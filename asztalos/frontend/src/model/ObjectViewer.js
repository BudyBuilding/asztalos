...
import { getAllCreatedItems, getAllObjects, getImageById } from "../data/getters";

export default function ObjectViewer({ object: objectProp, createdItems: itemsProp, usedColors, onItemUpdate }) {
...
const engineRef = useRef(null);
  ...
  const onHeaderMouseDown = e => { e.preventDefault(); setDragging(true); dragRef.current = { startX:e.clientX, startY:e.clientY, origX:modalPos.x, origY:modalPos.y }; };
  useEffect(() => {
    const onMouseMove = e => { if(!dragging) return; const dx=e.clientX-dragRef.current.startX; const dy=e.clientY-dragRef.current.startY; setModalPos({ x:dragRef.current.origX+dx, y:dragRef.current.origY+dy }); };
  ...
  if (!pickInfo.hit) {
    ...
    Object.values(groupNodesRef.current).forEach(node =>
      node.getChildMeshes().forEach(mesh =>
        mesh.material && (mesh.material.diffuseColor = new Color3(0.7, 0.7, 0.7))
      )
    );
    ...
    const camera = new ArcRotateCamera("cam", Math.PI/2, Math.PI/4, 25, Vector3.Zero(), scene);
    ...
    new HemisphericLight("ground", new Vector3(0,-1,0), scene).intensity=0.2;
...
const didInitialFit = useRef(false);
  ...
  useEffect(() => {
    ...
    const maxDim = Math.max(...createdItems.map(it => parseSizeString(it.size)).flat(), 1);
    ...
    let parentNode = null;
      ...
      const w = rw * globalScale, h = rh * globalScale, d = rd * globalScale;
      ...
      rawPosList.forEach(([x0, y0, z0], instIdx) => {
        ...
        if (item.colorId != null) {
          ...
          if (col) {
            const img = dispatch(getImageById(col.imageId));
        ...
        mesh.actionManager.registerAction(
          new ExecuteCodeAction(ActionManager.OnLeftPickTrigger, () => {
            // Reset highlights
            Object.values(groupNodesRef.current).forEach(node =>
              node.getChildMeshes().forEach(m2 => m2.material.diffuseColor = new Color3(0.7, 0.7, 0.7))
            );
...
//              setPicked(p => ({ ...p, groupMode: true, groupId, groupIdx: null, instIdx: null }));
              ...
              node.getChildMeshes().forEach(m2 => m2.material.diffuseColor = new Color3(1, 0, 0));
              setIsModalOpen(true);
              return;
            }
            else {
            ...
            setPicked(p => ({ ...p, groupMode: false, groupId: null, groupIdx: gIdx, instIdx: iIdx }));
...
const scheduled = { current: false };
  ...
  useEffect(()=>{
    ...
    const globalScale=1/Math.max(...createdItems.map(it=>parseSizeString(it.size)).flat(),1);
    node.position = new Vector3(groupDelta.x*globalScale, groupDelta.y*globalScale, groupDelta.z*globalScale);
  ...
  const mesh = sceneRef.current.getMeshByName(`box_${selectedGroupIdx}_${selectedInstIdx}`);
  if (!mesh) return;

  const scale = 1 / Math.max(...createdItems.map(it => parseSizeString(it.size)).flat(), 1);
...
}, [itemPosition, itemRotation, isGroupMode, selectedGroupIdx, selectedInstIdx, createdItems]);
  ...
  const handleSave = () => {
    if (isGroupMode && selectedGroupId !== "0") {
      createdItems.forEach((orig, idx) => {
        ...
        const newPosList = posList.map(([x,y,z]) =>
          [x+groupDelta.x, y+groupDelta.y, z+groupDelta.z]
        );
        const newRotList = rotList.map(([rx,ry,rz]) =>
          [rx+groupRotation.x, ry+groupRotation.y, rz+groupRotation.z]
        );
    ...
    onItemUpdate({...orig, position:newPos, rotation:newRot}, selectedGroupIdx);
    setIsModalOpen(false);
  };
...
/>
    <Modal show={isModalOpen} onHide={()=>setIsModalOpen(false)} backdropClassName="no-blur-backdrop" centered={false}
      dialogAs={({className,style,children,...props})=>(<div {...props} className={className} style={{...style,position:"absolute",width: "15vw", top:modalPos.y,left:modalPos.x,margin:0,backgroundColor:"#fff",borderRadius:"0.3rem",boxShadow:"0 2px 10px rgba(0,0,0,0.2)"}}>{children}</div>)}>
      <Modal.Header closeButton onMouseDown={onHeaderMouseDown} style={{cursor:"move"}}>
      
      <Modal.Title style={{ flex: 1, margin: 0 }}>
        {isGroupMode ? "Move Group" : `Edit Item #${selectedGroupIdx}:${selectedInstIdx}`}
      </Modal.Title>
...
<Modal.Body>
  {selectedGroupId && (
      <Button
        ...
        onClick={() => {
          ...
          Object.values(groupNodesRef.current).forEach(node =>
            node.getChildMeshes().forEach(m => m.material.diffuseColor = new Color3(0.7, 0.7, 0.7))
          );
          if (nextMode) {
            // csoport mód: highlight a kijelölt csoport
            groupNodesRef.current[selectedGroupId]?.getChildMeshes()
              .forEach(m => m.material.diffuseColor = new Color3(1, 5, 0));
          } else {
            ...
            const pos = parseBracketList(createdItems[gIdx].position, createdItems[gIdx].qty)[iIdx];
            setItemPosition({ x: pos[0], y: pos[1], z: pos[2] });
            const rot = parseBracketList(createdItems[gIdx].rotation, createdItems[gIdx].qty)[iIdx];
            setItemRotation({ x: rot[0], y: rot[1], z: rot[2] });
            // single mesh highlight
            const mesh = sceneRef.current.getMeshByName(`box_${gIdx}_${iIdx}`);
            mesh?.material && (mesh.material.diffuseColor = new Color3(1, 5, 0));
          }
  ...
  {["x","y","z"].map(ax=>(
    <div key={ax} style={{marginBottom:12}}>
      <label>{`${ax.toUpperCase()}: ${isGroupMode?groupDelta[ax]:itemPosition[ax]}`}</label>
      <input
        type="range"
     ...
     {["x","y","z"].map(ax=>(
      ...
      <Modal.Footer><Button variant="secondary" onClick={()=>setIsModalOpen(false)}>Cancel</Button><Button variant="primary" onClick={handleSave}>Save</Button></Modal.Footer>
    </Modal>
  </>);
}

// Helpers
function parseSizeString(input){if(Array.isArray(input)){const nums=input.map(n=>parseFloat(n)||0);return nums.length===3?nums:[1,1,1];}if(typeof input!="string")return[1,1,1];return input.slice(1,-1).split(",").map(n=>parseFloat(n)||0);}function parseBracketList(input="",qty){if(Array.isArray(input))return input.slice(0,qty).map(a=>Array.isArray(a)?a.map(n=>parseFloat(n)||0):[0,0,0]);const matches=(input.match(/\[.*?\]/g)||[]).slice(0,qty);return Array.from({length:qty},(_,i)=>{const m=matches[i];return m?m.slice(1,-1).split(",").map(n=>parseFloat(n)||0):[0,0,0];});}
