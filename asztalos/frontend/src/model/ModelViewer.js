...
export default function ModelViewer({
  ...
  useEffect(() => {
    ...
    new HemisphericLight("hemilight", new Vector3(0, 1, 0), scene).intensity =
      0.7;
    new PointLight("filllight", new Vector3(2.5, 2, 2.5), scene).intensity = 0.3;
...
/*
  ...
  useEffect(() => {
    ...
    createWall("room_leftWall", { width: thin, height: roomH, depth: roomD }, [thin / 2, roomH / 2, roomD / 2]);
    createWall("room_rightWall", { width: thin, height: roomH, depth: roomD }, [roomW - thin / 2, roomH / 2, roomD / 2]);
    createWall("room_frontWall", { width: roomW, height: roomH, depth: thin }, [roomW / 2, roomH / 2, thin / 2]);
    createWall("room_backWall", { width: roomW, height: roomH, depth: thin }, [roomW / 2, roomH / 2, roomD - thin / 2]);
    createWall("room_ceiling", { width: roomW, height: thin, depth: roomD }, [roomW / 2, roomH - thin / 2, roomD / 2]);
    ...
    createdItems.forEach((item) => {
      ...
      if (!groupRootsRef.current[oid]) {
        ...
        const [opx, opy, opz] = parsePositionString(objData.position || "[0,0,0]", 1)[0];
        const [orx, ory, orz] = parseRotationString(objData.rotation || "[0,0,0]", 1)[0];
    ...
    createdItems.forEach((item, gi) => {
      ...
      const w = rw * objectScale, h = rh * objectScale, d = rd * objectScale;
      ...
      rawPos.forEach(([x0, y0, z0], idx) => {
        ...
        mesh.rotation = new Vector3(rx * Math.PI/2, ry * Math.PI/2, rz * Math.PI/2);
        ...
        if (col) {
          ...
          if (img) mat.diffuseTexture = new Texture(`data:image/jpeg;base64,${img}`, scene);
  ...
  const handlePositionChange = (ax, value) => {
    ...
    const item = createdItems.find(i => i.object.objectId === selectedObjectId);
    if (!item) return;

    const roomWmm = 5 * 1000, roomDmm = 5 * 1000, roomHmm = 2 * 1000;
    const allSizes = createdItems.map(i => parseSizeString(i.size));
    const maxDim = Math.max(...allSizes.flat(), 1);
    ...
    let newX = objPosition.x, newY = objPosition.y, newZ = objPosition.z;
    if (ax === "x") {
      const half = wmm/2;
...
const handleSave = () => {
  if (selectedObjectId != null) {
    ...
    if (obj) {
      ...
      const updatedObject = {
        ...obj,
        position: `[${(objPosition.x/1000).toFixed(3)},${(objPosition.y/1000).toFixed(3)},${(objPosition.z/1000).toFixed(3)}]`,
        rotation: `[${objRotation.x},${objRotation.y},${objRotation.z}]`
      };
      // Notify parent to update the object, passing the object index
      const objectIndex = objects.findIndex(o => o.objectId === selectedObjectId);
          ...
          {["x","y","z"].map((ax) => (
            <Form.Group key={ax} className="mb-3">
              ...
              <Form.Range
                ...
                onInput={(e) => handlePositionChange(ax, parseFloat(e.target.value))}
              />
            </Form.Group>
          ))}
          {["x","y","z"].map((ax) => (
...
function parseSizeString(input) {
  ...
  return input.slice(1, -1).split(",").map((n) => parseFloat(n) || 0);
}

function parseBracketList(input = "", qty) {
  if (Array.isArray(input)) {
    return input.slice(0, qty).map((a) =>
  ...
  return Array.from({ length: qty }, (_, i) => {
    const m = matches[i];
    return m
      ? m.slice(1, -1).split(",").map((n) => parseFloat(n) || 0)
      : [0, 0, 0];
  });
}
...
function parsePositionString(input = "", qty) {
  ...
  return matches.slice(0, qty).map((m) =>
    m.slice(1, -1).split(",").map((n) => parseFloat(n.trim()) || 0)
  );
}
